...
In order to reset the tamagotchi, pull the rst_n low for a while (if the 'A' button is pressed while rst_n is low, the tamagotchi enters a 'fast' mode where the battery depletes quicker, making testing easier).
When the rst_n is high again, the screen should flash colorfully as the the cat is born. After this process, the tamagotchi is in a 'default' idle state. Use the buttons on the gamepad to traverse states, discover the fish-catching minigame and keep the cat alive.
